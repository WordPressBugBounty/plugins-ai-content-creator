...
if ( isset( $_POST['generar'] ) ) {
		// Verificar el nonce.
	if ( ! isset( $_POST['aicc_nonce'] ) || ! check_admin_referer( 'aicc_crear_articulo_nonce', 'aicc_nonce', ) ) {
		wp_die( 'Acceso a crear artículo no autorizado.' );
	}
	$generar = true;
...
if ( $regenerar ) {
	...
	*/
	...
	aicc_mostrar_imagen_espera_html();
?>
...
ocultaEspera();
	...
	if ( ! $aicc_generacion->contenido() ) {
		...
		echo wp_kses( $aicc_generacion->bloque_operaciones_html(), $aicc_generacion->aicc_permitidos_kses_operaciones() );
	}
	echo '<hr>';
}
...
function aicc_formulario_crear_html( $titulo, $contexto, $directrices_adicionales, $deshabilitado ) {
	...
	$html .= wp_nonce_field( 'aicc_crear_articulo_nonce', 'aicc_nonce' );
...
/**
 * Muestra el HTML de la imagen de espera y el script asociado para manejar su visibilidad.
 *
 * Esta función genera y muestra el código HTML necesario para mostrar una imagen de espera
 * (por ejemplo, un spinner o un indicador de carga) y un script de JavaScript que controla
 * la visibilidad de la imagen y otros elementos relacionados.
 *
 * El contenedor de la imagen (`<div id="espera">`) está oculto inicialmente (`display: none;`),
 * y el script `mostrarEspera()` se encarga de mostrarlo y ocultar otros elementos cuando sea necesario.
 *
 * @since 1.0.0
 *
 * @return void Esta función no devuelve ningún valor, ya que imprime directamente el HTML y el script.
 */
function aicc_mostrar_imagen_espera_html() {
	?>
	<script>
	...
	<!-- Para verlo en pruebas, ponerlo en display: block, en producción en display: none. -->
	<div id="espera" style="display: none; text-align: center; margin-top: 20px;">
		<div style="display: inline-block; width: 150px; position: relative;">
			<img src="<?php echo esc_attr( plugin_dir_url( __FILE__ ) ); ?>images/ia.svg" width="150" height="150" style="width:150px; height:150px;" alt="<?php esc_attr_e( 'Creating article...', 'ai-content-creator' ); ?>">
		</div>
		<p style="margin-top: 20px;"><?php esc_html_e( 'Creating article...', 'ai-content-creator' ); ?></p>
	</div>
	<?php
}
