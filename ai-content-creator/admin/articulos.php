...
function aicc_mostrar_pagina_articulos() {
	...
	if ( isset( $_GET['id'] ) ) {
		$article_id = aicc_valida_id_articulo( sanitize_text_field( wp_unslash( $_GET['id'] ) ) );
		if ( ! $article_id ) {
	...
	if ( isset( $_GET['accion'] ) ) {
		$accion = aicc_filtra_accion_articulos( sanitize_text_field( wp_unslash( $_GET['accion'] ) ) );
		if ( ! $accion ) {
	...
	if ( isset( $_GET['imagen_url'] ) ) {
		$imagen_url = esc_url( sanitize_text_field( wp_unslash( $_GET['imagen_url'] ) ) );
		if ( ! $imagen_url ) {
	...
	if ( isset( $_GET['keywords'] ) ) {
		$keywords = aicc_sanea_keywords_pixabay( sanitize_text_field( wp_unslash( $_GET['keywords'] ) ) );
	}
...
function aicc_borra_articulo( $article_id ) {
	...
	if ( isset( $_GET['campo_nonce'] )
		&& wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['campo_nonce'] ) ), 'borrado' )
	) {
